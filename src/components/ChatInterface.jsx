...
// Helper function to detect if input is a supported platform URL (Airbnb or Booking.com)
const isSupportedUrl = (text) => {
  return isAirbnbUrl(text) || isBookingUrl(text);
};
...
const ChatInterface = ({ me: meProp, meLoading: meLoadingProp }) => {
 ...
 // Store scan data for sidebar display
  ...
  const handleCompare = async (text) => {
    ...
    if (isCompareRequest) {
      ...
      // Build available scans list (fast - no API calls, use existing data)
      const availableScans = scanChats.map(chat => {
        ...
        // Extract URL from title (format: "Scan • {url}")
        const listingUrl = chat.title.replace("Scan • ", "").trim();
      ...
      setAvailableScansForComparison(availableScans);
      
      // Load scan data in background (lazy) for better titles, but don't block UI
      scanChats.forEach(chat => {
        const scan = getScanDataFromCurrentMessages(chat.id) || scanData[chat.id];
        if (!scan && chat.type === 'scan') {
          // Load in background without blocking
          loadScanDataForChat(chat.id).then(loadedScan => {
            if (loadedScan) {
              // Update the available scans list with better data
              setAvailableScansForComparison(prev => prev.map(s => 
                s.id === chat.id 
                  ? { ...s, listing_title: loadedScan.listing_title, location: loadedScan.location }
                  : s
              ));
            }
          });
  ...
  const handleSubmit = async (e) => {
    ...
    if (isCompareRequest(trimmedInput)) {
      ...
      setMessages(prev => [...prev, {
        role: "assistant",
        content: "The URL you are trying to scan is from a platform that we do not cover yet. We currently support Airbnb and Booking.com. Bear with us as we are expanding quickly.",
        isError: true
      }]);
    } else {
